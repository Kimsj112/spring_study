package com.example.mybatis2.mybatis;
...
//        DataSource란?
//        DataSource객체는 CP(커넥션 풀)을 관리하고 CP에 있는 커넥션 객체를 제공/반납받는 등의 일을 한다.
//        모든 CP 라이브러리는 DataSource객체를 사용한다.
//        이 객체가 데이터베이스 커넥션을 관리, 풀을 통한 재사용을 담당하며 CP을 사용하려면 항상 DataSource객체가 필요한다.
...
//        그렇게 만들어진 SqlSessionFactory를 스프링 컨테이거가 관리한다.
        SqlSessionFactoryBean sqlSessionFactoryBean = new SqlSessionFactoryBean();
        sqlSessionFactoryBean.setDataSource(dataSource());

//        매퍼 파일 경로를 알려주기 위한 설정
//        applicationContext 객체는 현재 프로젝트의 설정, 파일 경로 등을 알고 있다.
//        classpath*는 java 디렉터리와 resources디렉터리 에서 검색하여 일치하는 모든 파일을 찾아낸다.(여러파일)
//        해당 패턴으로 검색된 파일들의 경로를 sqlSessionFactoryBean에 저장한다.
        sqlSessionFactoryBean.setMapperLocations(applicationContext.getResources("classpath*:/mapper/**/*.xml"));

//        config.xml의 경로를 알려준다.
//        classpath 는 하나의 파일만 검색하여 찾는다.
        sqlSessionFactoryBean.setConfigLocation(applicationContext.getResource("classpath:/config/config.xml"));
        sqlSessionFactoryBean.setTypeAliasesPackage("com.example.mybatis2.dto");
